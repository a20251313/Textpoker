...
#define TOKENSERVER_PROTOCOL_ID               20121115
...
public:
    ...
    {        
        ...
        ePT_TA_Hello                      = 1100,
        ePT_TA_TokenUpdateAck             = 1101,
        ePT_TA_ResetBadgeAck              = 1102,
    ...
    typedef enum
    {
        ...
        PUSH_SERVICE_IOS_TOKEN_SIZE          =  32,
...
public:
    PacketHandler(unsigned int user_id, unsigned int app_id, unsigned int language, unsigned int platform);
    ~PacketHandler();
...
///  Token server.
///////////
    bool analyzeTokenServerHandshakePkt(const unsigned char *buff, unsigned int bufflen);
    bool createTokenUpdatePkt(int add_del, unsigned char *token, unsigned char *buff, unsigned int max_buff_len, unsigned int &bufflen);
    bool createBadgeResetPkt(unsigned char *buff, unsigned int max_buff_len, unsigned int &bufflen);
    bool analyzeTokenUpdateAck(const unsigned char *buff, unsigned int bufflen);
    bool analyzeResetBadgeAck(const unsigned char *buff, unsigned int bufflen);
