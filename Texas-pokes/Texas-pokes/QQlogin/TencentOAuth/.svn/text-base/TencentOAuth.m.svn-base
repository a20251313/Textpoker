...
static NSString *kWeiboRestserverBaseURL = @"https://graph.qq.com/t/";
...
#pragma mark - QQlogin
#ifndef QQAPP_LOGIN_KEY_INFO
#define QQAPP_LOGIN_KEY_INFO
#define QQLoginAPPKEY        @"100340340"
#define QQLoginAPPSECRET    @"13640671c139f82742168a13cd1a9c35"
#define QQLoginKeyURL       @"www.qq.com"
#endif
...
///////////////////////////////////////////////////////////////////////////////////////////////////
static TencentOAuth   *tencent = nil;
...
+ (id)ShareInstance:(id)Delegate
{
    if (tencent == nil)
    {
        //@"100674991"
        tencent = [[TencentOAuth alloc] initWithAppId:QQLoginAPPKEY
                                                andDelegate:Delegate];
        
        NSUserDefaults *standUserdefault = [NSUserDefaults standardUserDefaults];
        NSDictionary  *dicInfo = [standUserdefault valueForKey:@"QQLOGINAPPINO"];
        if (dicInfo && [dicInfo valueForKey:@"accessToken"] && [dicInfo valueForKey:@"expirationDate"] && [dicInfo valueForKey:@"OPENID"])
        {
            tencent.accessToken = [dicInfo valueForKey:@"accessToken"];
            tencent.expirationDate = [dicInfo valueForKey:@"expirationDate"];
            tencent.openId = [dicInfo valueForKey:@"OPENID"];
        }
        // NSDictionary  *dicInfo = [NSDictionary dictionaryWithObjectsAndKeys:self.accessToken,@"accessToken",self.expirationDate,@"expirationDate",self.openId,@"OPENID",nil];
       // [standUserdefault setObject:dicInfo forKey:@"QQLOGINAPPINO"];
       // [standUserdefault synchronize];
        tencent.redirectURI = QQLoginKeyURL;
    }
    tencent.sessionDelegate = Delegate;
   // [tencent setSessionDelegate:Delegate];
    return tencent;
}
				...
				andDelegate:(id<TencentSessionDelegate>)delegate
{
	...
	if (self)
	{
		...
		_redirectURI = [[NSString alloc] initWithString:QQLoginKeyURL];
...
- (BOOL)getUserInfo
{
	...
	APIBase *apiRequest = [[APIBase alloc] initWithTarget:self
                                              andSelector:@selector(getUserInfoResponse:)
                                            andHttpMethod:@"GET"];
 ...
 * Add  message
 */
- (BOOL)addMessageWithParams:(NSMutableDictionary *)params
{
	if (![self isSessionValid] || ![self isOpenIdValid]) {
		return NO;
	}
	
	APIBase *apiRequest = [[APIBase alloc] initWithTarget:self andSelector:@selector(addMessageResponse:) andHttpMethod:@"POST"];
	NSString * fullURL = [kWeiboRestserverBaseURL stringByAppendingString:@"add_t"];
	[apiRequest openUrl:fullURL token:_accessToken openid:_openId appid:_appId params:params];
	[_apiRequests setObject:apiRequest forKey:[NSString stringWithFormat:@"%d", apiRequest.seq]];
   // NSLog(@"url:%@ content:%@",fullURL,params);
	return YES;
}
...
-(void)addMessageResponse:(APIResponse*) response
{
    if (!response)
		return;
    NSLog(@"jsonResponse:%@message: %@  %d",response.jsonResponse,response.message,response.retCode);
	[self deleteAPIRequestBySeq:[NSString stringWithFormat:@"%d", response.seq]];
	if ([self.sessionDelegate respondsToSelector:@selector(addMessageResponse:)])
	{
		[_sessionDelegate addMessageResponse:response];
	}
    
}


- (BOOL)addMessageAndPicWithParams:(NSMutableDictionary *)params
{
	if (![self isSessionValid] || ![self isOpenIdValid]) {
		return NO;
	}
	
	APIBase *apiRequest = [[APIBase alloc] initWithTarget:self andSelector:@selector(addMessageAndPicResponse:) andHttpMethod:@"POST"];
	NSString * fullURL = [kWeiboRestserverBaseURL stringByAppendingString:@"add_pic_t"];
	[apiRequest openUrl:fullURL token:_accessToken openid:_openId appid:_appId params:params];
	[_apiRequests setObject:apiRequest forKey:[NSString stringWithFormat:@"%d", apiRequest.seq]];
    NSLog(@"url:%@ content:%@",fullURL,params);
	return YES;
...
-(void)addMessageAndPicResponse:(APIResponse*) response
{
    ...
    NSLog(@"jsonResponse:%@message: %@  %d",response.jsonResponse,response.message,response.retCode);
	[self deleteAPIRequestBySeq:[NSString stringWithFormat:@"%d", response.seq]];
     if ([self.sessionDelegate respondsToSelector:@selector(addMessageAndPicResponse:)])
     {
     [_sessionDelegate addMessageAndPicResponse:response];
     }
...
- (void)tencentDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate
{
	...
	// 继续获取openid
   // NSMutableDictionary   *dicInfo = [[NSUserDefaults standardUserDefaults] valueForKey:<#(NSString *)#>];
	NSMutableDictionary* params = [NSMutableDictionary dictionaryWithObjectsAndKeys:
																	token, @"access_token",nil];
    NSLog(@"accessToken:%@ expireationDate:%@",self.accessToken,self.expirationDate);
...
- (void)request:(TencentRequest *)request didLoad:(id)result dat:(NSData *)data
{
	...
	if ([self.sessionDelegate respondsToSelector:@selector(tencentDidLogin:)])
    {
		[_sessionDelegate tencentDidLogin:root];
        
	}
	NSLog(@"received didLoad success  openid=%@", openid);
};
