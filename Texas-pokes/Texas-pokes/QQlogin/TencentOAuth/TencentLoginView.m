...
static BOOL TencentIsDeviceIPad()
{
...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
	...
	if (!([error.domain isEqualToString:@"WebKitErrorDomain"] && error.code == 102)) {
        ...
        {
			[(id)_delegate tencentDidNotNetWork];
		}	
...
- (void)keyboardWillShow:(NSNotification*)notification {
    ...
    if ([[[notification userInfo] valueForKey:@"UIKeyboardAnimationDurationUserInfoKey"] floatValue] <= 0.2)
    {
        return;
    }
	UIInterfaceOrientation orientation = [UIApplication sharedApplication].statusBarOrientation;
	if (UIInterfaceOrientationIsLandscape(orientation))
    {
        ...
        NSLog(@"webValue frame:%@ kpadding:%0.0f kBorderWidth:%0.0f _titleLabel.frame.size.heigh:%0.0f ",[NSValue valueWithCGRect:_webView.frame],kPadding,kBorderWidth,_titleLabel.frame.size.height);
