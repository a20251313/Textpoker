...
static SinaWeibo  *instanceSinaWeibo = nil;
@interface SinaWeibo ()
...
+(id)shareInstance:(id)Delegate
{
    ...
    if (instanceSinaWeibo == nil)
    {
        instanceSinaWeibo = [[SinaWeibo alloc] initWithAppKey:kWBSDKDemoAppKey
                                                    appSecret:kWBSDKDemoAppSecret
                                               appRedirectURI:kWBSDKDemoURL
                                                  andDelegate:Delegate];
        NSUserDefaults  *standDefault = [NSUserDefaults standardUserDefaults];
        NSDictionary  *dicInfo = [standDefault valueForKey:@"SinaWeiboAuthData"];
        if (dicInfo && [dicInfo valueForKey:@"AccessTokenKey"] && [dicInfo valueForKey:@"ExpirationDateKey"] && [dicInfo valueForKey:@"UserIDKey"])
        {
            instanceSinaWeibo.accessToken = [dicInfo valueForKey:@"AccessTokenKey"];
            instanceSinaWeibo.expirationDate = [dicInfo valueForKey:@"ExpirationDateKey"];
            instanceSinaWeibo.userID = [dicInfo valueForKey:@"UserIDKey"];
            instanceSinaWeibo.refreshToken = [dicInfo valueForKey:@"refresh_token"];
        }
    }
    if (Delegate != nil)
    {
        instanceSinaWeibo.delegate = Delegate;
    }
    
    return instanceSinaWeibo;
}
...
-(void)PublicWeiBoMessage:(NSString *)strMessage
{
    [self requestWithURL:@"statuses/update.json"
                  params:[NSMutableDictionary dictionaryWithObject:strMessage forKey:@"status"]
              httpMethod:@"POST"
                delegate:self];
}
-(void)PublicWeiBoMessage:(NSString *)strMessage withImage:(id)Thread
{
    [self requestWithURL:@"statuses/upload.json"
                  params:[NSMutableDictionary dictionaryWithObjectsAndKeys:strMessage,@"status",Thread,@"pic", nil]
              httpMethod:@"POST"
                delegate:self];
}
...
- (void)request:(SinaWeiboRequest *)_request didFailWithError:(NSError *)error
{
    ...
    if ([delegate respondsToSelector:@selector(sinaweiboDataResponse:withErrorInfo:withRequesttype:)])
    {
       [delegate sinaweiboDataResponse:_request withErrorInfo:error withRequesttype:-1];
    }
   
}


// 1 userInfo  2 message   3  pic
- (void)request:(SinaWeiboRequest *)_request didFinishLoadingWithResult:(id)result
{
    ...
    if ([_request.url hasSuffix:@"users/show.json"])
    {
        ...
        if ([delegate respondsToSelector:@selector(sinaweiboDataResponse:withErrorInfo:withRequesttype:)])
        {
             [delegate sinaweiboDataResponse:result withErrorInfo:nil withRequesttype:1];
        }
      
    }else if ([_request.url hasSuffix:@"statuses/update.json"])
    {
        if ([delegate respondsToSelector:@selector(sinaweiboDataResponse:withErrorInfo:withRequesttype:)])
        {
            [delegate sinaweiboDataResponse:result withErrorInfo:nil withRequesttype:2];
        }
    }else if ([_request.url hasSuffix:@"statuses/upload.json"])
    {
        if ([result valueForKey:@"error"] && [[result valueForKey:@"error_code"] isEqual:@"20019"])
        {
            NSLog(@"error:%@",result);
        }
        if ([delegate respondsToSelector:@selector(sinaweiboDataResponse:withErrorInfo:withRequesttype:)])
        {
            [delegate sinaweiboDataResponse:result withErrorInfo:nil withRequesttype:3];
        }
    }
       NSLog(@"result:%@",result);
}
...
{
    return [NSString stringWithFormat:@"..............self address:%p accessToken:%@ refreshToken:%@ uid:%@..............",self,self.accessToken,self.expirationDate,self.userID];
}
@end
