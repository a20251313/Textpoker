...
@class RJFChargeViewController;
//NSString *const    BNRHUDREMAINTOOLONG = @"BNRHUDREMAINTOOLONG";
...
-(void)checkHidden:(id)Timer
{
    if (self.superview != nil)
    {
      [self removeFromSuperview];
      [[NSNotificationCenter defaultCenter] postNotificationName:BNRDISCONNECTSERVER
                                                          object:self];
    }
   // [self removeFromSuperview];
   // NSLog(@"timer:%@  self:%@ ------------hidden my hud--------------\n\n hidden:%d superview:%@",self,[NSDate date],self.hidden,self.superview);
}
...
- (void)show:(BOOL)animated  timeout:(NSTimeInterval)timeout
{
	useAnimation = animated;
    ...
    //return;
        ...
        NSTimer  *timer = [[NSTimer alloc] initWithFireDate:[NSDate dateWithTimeInterval:timeout sinceDate:[NSDate date]]
                                                   interval:0
                                                     target:self
