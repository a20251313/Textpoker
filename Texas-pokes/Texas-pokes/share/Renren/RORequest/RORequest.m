...
#ifdef DEBUG_VERSION
                logMyLogInfo(@"can not use GET to upload a file");
#endif
            }
            continue;
        }
...
- (void)connect{
	...
	/*
     ...
     autorelease];
     logMyLogInfo(@"======：%@",responseString);
     */
	//logMyLogInfo(@"Here's the request headers: %@", [urlRequest allHTTPHeaderFields]);
    //logMyLogInfo(@"Here's the request body: %@", [urlRequest HTTPBody]);
...
- (id)parseJsonResponse:(NSData *)data error:(NSError **)error{
    ...
    NSString* responseString = [[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding] autorelease];
#ifdef DEBUG_VERSION
    logMyLogInfo(@"Here's the response string: %@", responseString);
#endif
    SBJSON *jsonParser = [[SBJSON new] autorelease];
