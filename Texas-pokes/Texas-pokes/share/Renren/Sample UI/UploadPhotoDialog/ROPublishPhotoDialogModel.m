...
#ifdef DEBUG_VERSION
        logMyLogInfo(@"%@",item.responseDictionary);
#endif
        [self uploadSuccess];
    }
...
#ifdef DEBUG_VERSION
    logMyLogInfo(@"Upload Photo Dialog View Error:%@",[error localizedDescription]);
#endif
...

{
    if (renren == self.renren) {
        ...
        [self.dialogView didRenrenAuthorizeSuccess:NO];
#ifdef DEBUG_VERSION
        logMyLogInfo(@"%@",error.localizedDescription);
#endif
    }
