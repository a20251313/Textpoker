...
-(I366_SERVER_UPLOAD_PCHC_HEADER )i366HeadStructInit:(id)info
{
    ...
    client_down.Version = DZPKPROTROLVERSION;
    client_down.ClientPlatform = DZPKCLINETPLATFORMFORi366Header;
...
-(void)i366PushHeadStruct:(size_t)size request:(int16_t)requestCode
{
    ...
    [self pushByte:'H'];
    [self pushByte:'C'];
    [self pushByte:DZPKPROTROLVERSION];
    [self pushByte:255];
    [self pushByte:DZPKLANGUAGE];
    [self pushByte:DZPKCLINETPLATFORMFORi366Header];
...
-(void)HearStructInit:(id)Info
{
    ...
    m_pro_Header.protolVersion = DZPKPROTROLVERSION;
